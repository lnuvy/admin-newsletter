...
interface KeywordEditableProps {
  ...
  putAction: (id: number, formData: FormData) => void
  deleteAction: (id: number) => void
}

const KeywordEditable = (props: KeywordEditableProps) => {
  const { keyword, putAction, deleteAction } = props
  ...
  if (isEdit) {
    return (
      <form action={(formData) => putAction(keyword.id, formData)} className="flex flex-wrap items-center gap-4">
        <input
          type="text"
          name="keyword_name"
